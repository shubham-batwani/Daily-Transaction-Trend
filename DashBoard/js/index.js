...
function onClickChangeData(freq){
  
  dataToPredict = freq.value ;
...
function onClickFetchData(){
  ...
  let requestUrl = "http://localhost:8787/ai";
    ...
    ,function(data){
      ...
      if(daily){
        ...
        daily.forEach(element => {
          ...
          data_raw.push({ timestamp : element['Date'] , price:parseFloat(element[dataToPredict])})
          console.log(element['Date'])
          console.log(element[dataToPredict])
...
function onClickDisplaySMA(){
  ...
  Plotly.newPlot( graph_plot, [{ x: timestamps_a, y: prices, name: "Actual Values" }], { margin: { t: 0 } } );
  Plotly.plot( graph_plot, [{ x: timestamps_b, y: sma, name: "SMA" }], { margin: { t: 0 } } );

  $("#div_linegraph_sma_title").text("Simple Moving Average (window: " + window_size + ")" );
  $("#btn_draw_sma").show();
  $("#load_draw_sma").hide();
...
async function onClickTrainModel(){
  ...
  console.log("my accuracy",result['stats'].history);
...
function onClickValidate() {
  ...
  let sma = sma_vec.map(function (val) { return val['avg']; });
  let prices = data_raw.map(function (val) { return val['price']; });
  sma_new = sma.slice(Math.floor(trainingsize / 100 * sma.length));
  let sma_len = (sma_new.length)*(-1);
  raw_new = prices.slice(sma_len);
  ...
  console.log("raw_new" , raw_new)
  console.log("val_y" , val_unseen_y)

  

  let customMAPE = calcMAPE(raw_new,val_unseen_y);

  console.log(customMAPE);


  const y = tf.tensor1d(raw_new);
  const z = tf.tensor1d(val_unseen_y);

  let acc = 100-customMAPE.toFixed(3);

  let txt = acc.toString();
  ...
  console.log('sma', sma , typeof sma)
...
async function onClickPredict() {
  ...
  let pred_X = [inputs[inputs.length-1]]; // 2 d array
  ...
  let pred_y = makePredictions([temp_X], result['model'], result['normalize']);
  ...
  // date
  let last_date = new Date(convertToDate(timestamps_d[timestamps_d.length-1]));
  
  last_date.setDate(last_date.getDate() + 1);
  let next_date = convertToIST(formatDate(last_date));
  ...
  {    
    ...
    console.log("1",temp_X)

    pred_y = makePredictions([temp_X], result['model'], result['normalize']);
    ...
    console.log("2",temp_X)

    last_date = new Date(convertToDate(timestamps_e[timestamps_e.length-1 ]));
    last_date.setDate(last_date.getDate()+1);
    next_date = convertToIST(formatDate(last_date));
    timestamps_e.push(next_date);
  ...
  Plotly.plot( graph_plot, [{ x: timestamps_e, y: Ans, name: "Predicted Trend" }], { margin: { t: 0 } } );
